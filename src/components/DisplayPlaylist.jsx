...
const DisplayPlaylist = () => {
    ...
    const { removePlaylist, fetchSinglePlaylist, updatePlaylistName } = useContext(PlaylistContext);
    const { playWithId ,playPlaylist, track } = useContext(PlayerContext); // To play songs from the list
    ...
    return (
        <div className="flex-1 h-full text-white">
            ...
            <div className="flex items-center gap-6 p-6 mt-4">
                <button 
                    onClick={() => playPlaylist(playlistData.songs , 0)}
            ...
            <div className="px-6 pb-20">
                {/* Table Header */}
                <div className="grid grid-cols-[16px_4fr_3fr_1fr] gap-4 px-4 py-2 border-b border-white/10 text-gray-400 text-sm mb-4 uppercase tracking-widest font-semibold">
                    <p>#</p>
                    <p>Title</p>
                    <p>Album</p>
                ...
                {playlistData.songs && playlistData.songs.length > 0 ? (
                    playlistData.songs.map((item, index) => (
                        <div 
                            ...
                            className="grid grid-cols-[16px_4fr_3fr_1fr] gap-4 px-4 py-2 rounded-md hover:bg-white/10 group transition-colors cursor-pointer items-center"
                        >
